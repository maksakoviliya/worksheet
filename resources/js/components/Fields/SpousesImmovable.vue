<template>
    <div id="income" class="rounded-lg bg-white p-6 mr-6 shadow-md">
        ...
        <template v-if="data.spousesImmovable.length">
            <div class="mt-4">
                <vuetable
                    ref="immovable"
                    :api-mode="false"
                    :css="classes.table"
                    :data="data.spousesImmovable"
                    :fields="fields"
                >
                    <div slot="actions" slot-scope="props">
                        <button
                            class="bg-gray-300 text-gray-600 p-1 rounded-lg hover:bg-gray-400 text-gray-700 focus:outline-none"
                            @click="showEditImmovableForm(props.rowIndex)">
                        ...
                        <button @click="data.spousesImmovable.splice(props.rowIndex, 1)"
                                class="bg-gray-300 text-gray-600 p-1 rounded-lg hover:bg-gray-400 text-gray-700 focus:outline-none">
            ...
            this.$modal.show(
                ...
                {classes: 'rounded-lg ml-32', height: 'auto', name: 'AddImmovableForm'},
                {
                    'immovableAdded': (immovable) => {
                        this.data.spousesImmovable.push(immovable)
                    }
                }
            )
        },
        showEditImmovableForm(index) {
            this.$modal.show(
                AddImmovableForm,
                {data: this.data.spousesImmovable[index]},
                {classes: 'rounded-lg ml-32', height: 'auto', name: 'AddImmovableForm'},
                {
                    'immovableEdited': (immovable) => {
                        this.data.spousesImmovable.splice(index, 1, immovable)
                    }
                }
            )
        }
