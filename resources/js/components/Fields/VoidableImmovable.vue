<template>
  <div id="voidable-movable" class="rounded-lg bg-white p-6 mr-6 shadow-md">
    <div class="flex items-center justify-between">
      <div>
        <h3 class="font-semibold text-lg text-gray-600 mr-4">Оспоримые сделки по недвижимому имуществу</h3>
      ...
      <button
          ...
          type="button" @click="showAddVoidableImmovableForm">Добавить
      </button>
    </div>
    <template v-if="data.voidableImmovable.length">
      <div class="mt-4">
        <vuetable
            ref="children"
            :api-mode="false"
            :css="classes.table"
            :data="data.voidableImmovable"
            :fields="fields"
        >
          <div slot="actions" slot-scope="props">
            <button
                class="bg-gray-300 text-gray-600 p-1 rounded-lg hover:bg-gray-400 text-gray-700 focus:outline-none"
                @click="showEditVoidableImmovableForm(props.rowIndex)">
            ...
            <button @click="data.voidableImmovable.splice(props.rowIndex, 1)"
                    class="bg-gray-300 text-gray-600 p-1 rounded-lg hover:bg-gray-400 text-gray-700 focus:outline-none">
...
import AddVoidableImmovableForm from "./forms/AddVoidableImmovableForm";
import Vuetable from "vuetable-2";

export default {
  name: "VoidableImmovable",
  components: {
    AddVoidableImmovableForm,
    Vuetable
  },
  props: [
    'data'
  ...
  methods: {
    showAddVoidableImmovableForm() {
      this.$modal.show(
          AddVoidableImmovableForm,
          {},
          {classes: 'rounded-lg ml-32', height: 'auto', width: '800px', name: 'AddVoidableImmovableForm'},
          {
            'voidableImmovableAdded': (voidable) => {
              this.data.voidableImmovable.push(voidable)
            }
          }
      )
    },
    showEditVoidableImmovableForm(index) {
      this.$modal.show(
          AddVoidableImmovableForm,
          {data: this.data.voidableImmovable[index]},
          {classes: 'rounded-lg ml-32', height: 'auto', width: '800px', name: 'AddVoidableImmovableForm'},
          {
            'voidableImmovableEdited': (immovable) => {
              this.data.voidableImmovable.splice(index, 1, immovable)
            }
          }
      )
    }
