<template>
    <div id="maritalStatus" class="rounded-lg bg-white p-6 mr-6 shadow-md">
            ...
            <label class="inline-flex items-center cursor-pointer" for="isMarried">
                    <span class="relative">
                      <span class="block w-10 h-6 bg-gray-400 rounded-full shadow-inner"></span>
                        <span
                            :class="isMarried ? 'bg-teal-500 transform translate-x-full' : 'bg-white'"
        ...
        <transition name="slide-y">
            <VueSlideToggle :duration="200" :open="isMarried" tag="div">
                <ValidationProvider v-slot="{ errors }" :class="isMarried ? 'opcity-1' : 'opacity-0'"
                                    class="mt-4" name="spouse" rules="required|max:255"
                                    tag="div">
                    <label class="inline-block text-gray-700 text-sm font-bold mb-2" for="spouse">ФИО супруга(и) в тварительном
                        падеже<span
                            class="text-red-300 text-sm">*</span>: <small v-if="errors[0]" class="text-red-400">{{
                                errors[0]
                            }}</small>
                    </label>
                    <input id="spouse" v-model="spouse"
