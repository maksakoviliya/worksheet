<template>
    <ValidationObserver ref="ValidationObserver" v-slot="{ invalid, handleSubmit }">
        <form @submit.prevent="handleSubmit(addUser)">
            <div>
                <ValidationProvider v-slot="{ errors }" name="username" rules="required|max:255">
            ...
            <div class="mt-4">
                <ValidationProvider v-slot="{ errors }" name="email" rules="required|email|max:255">
            ...
            <div class="mt-4">
                <ValidationProvider v-slot="{ errors }" name="password" rules="required|min:3|max:15">
            ...
            <transition name="slide-y">
                <VueSlideToggle :duration="200" :open="user.password.length > 0" tag="div">
                    <ValidationProvider v-slot="{ errors }" name="password_confirmation"
                                        rules="required|confirmed:password|max:15">
            ...
            <div class="mt-6">
                ...
                <button
                    ...
                    type="button" @click="$modal.hide('UsersCreateForm')">Отменить
                </button>
            </div>
        </form>
    </ValidationObserver>
    ...
    methods: {
        async addUser() {
            try {
                ...
                this.$parent.$emit('userAdded')
                this.$modal.hide('UsersCreateForm')
            } catch (e) {
                if (e.response.data.errors.hasOwnProperty('user.email')) {
